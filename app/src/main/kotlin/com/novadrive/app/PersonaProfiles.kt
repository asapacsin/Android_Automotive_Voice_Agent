...
""".trimIndent()
    ...
    const val FLEX_TOOL_RULE = "只能通过提供的工具执行导航、音乐、空调或打开支持的应用；不要假装已经执行；工具返回 ok=false 时必须如实说没有成功。导航进行中「保持安静」只表示不闲聊，用户提出的每个指令（例如「播放音乐」「关闭音乐」「换目的地」）仍然必须立即调用对应工具，绝不能不回应。没有对应工具的请求（例如调音量、开车窗）要用一句话说明暂时不支持，不要沉默。空调相关请求一律调用 control_climate，即使你认为已经到最高或最低，也要调用后根据返回的 limit_reached 回答。"
