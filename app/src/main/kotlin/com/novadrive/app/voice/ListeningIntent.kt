...
/**
 * Local interception of listening-control phrases before the model is involved. Whole utterances
 * only, never words inside a command: 「关闭小诺」 sleeps, 「关闭导航」「关闭空调」「close the window」 do not.
 *
 * Two different intents, never synonyms:
 * - [Decision.SHUT_UP] (「闭嘴」「stop talking」): cut off the reply, keep the conversation and keep
 *   listening (SILENT_WAIT);
 * - [Decision.GO_TO_SLEEP] (「休眠」「停止监听」「go to sleep」): stop listening (SLEEP).
 *
 ...
 * 3. Shut up / sleep phrases below.
 * 4. Everything else → model.
 */
object ListeningIntent {
    enum class Decision { SHUT_UP, GO_TO_SLEEP, PASS_TO_MODEL }
    ...
    /** Stop listening. Unambiguous in any context: they name sleep, listening or the assistant. */
    private val SLEEP = setOf(
        "sleep", "gotosleep", "stoplistening", "thatsall", "thatisall",
        "休眠", "进入休眠", "小诺休眠", "睡觉", "去睡觉", "睡吧", "去睡吧", "小诺睡觉", "小诺睡吧",
        "关闭小诺", "小诺关闭", "关掉小诺", "别听了", "不要听了", "停止监听", "停止聆听",
        "休息吧", "小诺休息吧", "你休息吧", "去休息吧", "小诺休息",
    )

    /** Stop talking now; the conversation goes on. */
    private val SHUT_UP = setOf(
        "shutup", "bequiet", "keepquiet", "keepsilent", "staysilent", "stoptalking", "silence", "quiet",
        "闭嘴", "小诺闭嘴", "安静", "安静点", "安静一点", "保持安静", "别说话", "不要说话", "别出声", "不要出声",
        "别吵", "别吵了", "少说话", "别说了", "不要说了", "别讲了", "不要讲了",
    )
    ...
    fun classify(utterance: String, context: Context): Decision {
        ...
        if (matches(phrase, SLEEP)) return Decision.GO_TO_SLEEP
        if (matches(phrase, SHUT_UP)) return Decision.SHUT_UP
        if (matches(phrase, CONTEXTUAL)) {
            return if (context.pickerOpen || context.taskPending) Decision.PASS_TO_MODEL else Decision.GO_TO_SLEEP
        }
