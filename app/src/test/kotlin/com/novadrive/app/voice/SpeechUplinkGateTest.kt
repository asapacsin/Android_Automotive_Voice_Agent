...
class SpeechUplinkGateTest {
    private val samplesPerFrame = 1600 // 100 ms at 16 kHz
    ...
    fun aSingleLoudImpulseNeverReachesTheModel() {
        ...
        // A tap: one 100 ms frame, very loud, then the room again.
    ...
    fun anImpulseIsReportedSoItCanBeCounted() {
        ...
        assertEquals(100, rejection.durationMs)
    }
    ...
    fun sustainedSpeechOpensTheGateAndKeepsStreaming() {
        val gate = SpeechUplinkGate()
        gate.offer(silence())
        assertTrue(gate.feed(tone(6_000)).isEmpty(), "one frame is not yet an onset")
    ...
    fun aClosedSegmentReportsItsShape() {
        ...
        repeat(12) { gate.offer(tone(6_000)) }
        var finished: SpeechUplinkGate.Segment? = null
        repeat(20) { if (finished == null) finished = gate.offer(silence()).finished }
        ...
        assertTrue(segment.voicedFrames >= 11)
    ...
    fun aShortBurstThatDoesOpenTheGateIsMarkedSuspicious() {
        ...
        val gate = SpeechUplinkGate()
        repeat(2) { gate.offer(tone(15_000)) }
        var finished: SpeechUplinkGate.Segment? = null
        repeat(20) { if (finished == null) finished = gate.offer(silence()).finished }
    ...
    fun aLongMostlySilentStretchIsHeldForJudgement() {
        val burst = SpeechUplinkGate.Segment(durationMs = 3_000, voicedFrames = 13, peak = 20_345)
        assertTrue(burst.needsHold(), "43% voiced over 3 s must be held")
        val sentence = SpeechUplinkGate.Segment(durationMs = 3_000, voicedFrames = 26, peak = 6_000)
