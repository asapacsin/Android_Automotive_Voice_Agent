...
class BaiduFlexProtocolTest {
    ...
    fun sessionUpdateContainsOnlyTwoTightlyControlledTools() {
        ...
        assertEquals(
            setOf("navigate_to", "open_app", "control_music", "control_climate", "describe_camera_view", "exit_navigation_mode", "choose_navigation_option", "end_conversation", "set_speech_output", "save_place", "place_call", "query_live_info"),
    ...
    @Test
    fun chooseNavigationOptionAcceptsExactlyOneValidField() {
        fun call(args: String): DomainVoiceEvent.ToolCall {
            val assembler = FlexFunctionCallAssembler().apply { consume(item("call_choose", "choose_navigation_option")) }
            return assembler.consume(done("call_choose", args)).single() as DomainVoiceEvent.ToolCall
        }
        assertEquals("2", call("""{"index":2}""").arguments["index"])
        assertEquals("fastest", call("""{"preference":"fastest"}""").arguments["preference"])
        assertEquals("拱北口岸", call("""{"name":"拱北口岸"}""").arguments["name"])
        assertEquals("INVALID_FIELDS", call("""{}""").arguments["_validation_error"])
        assertEquals("INVALID_FIELDS", call("""{"index":1,"preference":"fastest"}""").arguments["_validation_error"])
        assertEquals("INVALID_INDEX", call("""{"index":0}""").arguments["_validation_error"])
        assertEquals("INVALID_INDEX", call("""{"index":1.5}""").arguments["_validation_error"])
        assertEquals("PREFERENCE_NOT_ALLOWED", call("""{"preference":"scenic"}""").arguments["_validation_error"])
        assertEquals("INVALID_NAME", call("""{"name":" "}""").arguments["_validation_error"])
    }
    ...
    fun setSpeechOutputAcceptsOnlySilentOrSpoken() {
        fun call(args: String): DomainVoiceEvent.ToolCall {
            ...
            return assembler.consume(done("call_mode", args)).single() as DomainVoiceEvent.ToolCall
        }
        assertEquals("silent", call("""{"mode":"silent"}""").arguments["mode"])
        assertEquals("spoken", call("""{"mode":"spoken"}""").arguments["mode"])
        assertEquals("MODE_NOT_ALLOWED", call("""{"mode":"loud"}""").arguments["_validation_error"])
        assertEquals("INVALID_FIELDS", call("""{}""").arguments["_validation_error"])
    }
    ...
    fun sessionUpdateDefaultEmitsTunedVadThreshold() {
        ...
        assertEquals(12, session.getJSONArray("tools").length())
        assertEquals(300, turn.getInt("prefix_padding_ms"))
        assertEquals(200, turn.getInt("silence_duration_ms"))
    }
    ...
    /** SPEC-011: query_live_info is validated before it reaches the dispatcher. */
    @Test
    fun queryLiveInfoArgumentsAreValidated() {
        fun error(arguments: String) = (
            FlexFunctionCallAssembler().apply { consume(item("call_live", "query_live_info")) }
                .consume(done("call_live", arguments)).single() as DomainVoiceEvent.ToolCall
            ).arguments["_validation_error"]
        assertEquals(null, error("""{"kind":"weather","where":"珠海","day":"tomorrow"}"""))
        assertEquals(null, error("""{"kind":"along_route","category":"fuel"}"""))
        assertEquals("INVALID_KIND", error("""{"kind":"news"}"""))
        assertEquals("INVALID_FIELDS", error("""{"where":"珠海"}"""))
        assertEquals("INVALID_FIELDS", error("""{"kind":"weather","city":"珠海"}"""))
        assertEquals("INVALID_FIELD_TYPE", error("""{"kind":"place_details","target":2}"""))
    }
}
