...
| Call a contact | `place_call(contact)` | `PhoneCallTool` → `AndroidContacts` → `ACTION_CALL` | `ok=true status=confirm_required` with the **name only** — nothing is dialled yet. A second call with `confirmed=true` places it | `ok=false` `NO_TELEPHONY` (no SIM — device-verified), `CONTACT_NOT_FOUND`, or `status=ambiguous` when several people share the name |
