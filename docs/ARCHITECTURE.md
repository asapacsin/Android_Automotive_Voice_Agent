...
| Whether 小诺 may speak / whether the mic reaches Baidu | `SpeechArbiter` via `SpeechAuthority` (P1 window, guidance hold, focus, guidance uplink gate, workload hold R6a whose navigation input is being wired in SPEC-012 step 4), applied by `AndroidPlaybackPort` (+ lifecycle) + `PhantomTurnGate` (phantom/false-claim holds) | the UI |
